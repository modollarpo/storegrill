    ...
    <div ref={boxRef} className="relative flex-1 min-w-0 mx-1 md:mx-3 max-w-xl">
      ...
      {expanded && (rows.length > 0 || loading) && (
        <ul
          ...
          className="absolute left-0 right-0 top-full mt-2 z-[60] bg-surface rounded-lg shadow-elevated border border-border overflow-hidden py-1.5 animate-fade-in max-h-[70vh] overflow-y-auto"
        >
          {/* Loading skeletons */}
          {loading && query.trim() && (
            <>
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="flex items-center gap-2 mb-1">
                        ...
                        {row.rating && row.rating >= 4.5 && (
                          <span className="inline-flex items-center gap-1 shrink-0 bg-feedback-warning-bg text-feedback-warning-fg text-2xs font-bold px-1.5 py-0.5 rounded-full">
