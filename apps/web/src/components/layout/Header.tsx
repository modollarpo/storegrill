        ...
        >
          <div className="container-fluid">
            <div className="flex items-center h-[56px] gap-4 w-full justify-between">
              ...
              <div className="w-[160px] shrink-0 flex justify-end relative" ref={dealsDropdownRef}>
                <button
                  type="button"
                  onClick={() => setDealsDropdownOpen(prev => !prev)}
                  className="inline-flex items-center gap-1.5 text-[15px] xl:text-[16px] font-semibold text-secondary hover:opacity-80 transition-opacity whitespace-nowrap outline-none cursor-pointer"
                  aria-expanded={dealsDropdownOpen}
                >
                  <span>Today&apos;s Deals</span>
                  <svg
                    className={cn('w-4 h-4 transition-transform duration-200', dealsDropdownOpen && 'rotate-180')}
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2.5"
