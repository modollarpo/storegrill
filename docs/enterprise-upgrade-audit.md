...
- **Committed SSL certificate (�5b)**: storegrill-wildcard.pfx (private key) was removed from tracking and gitignored (*.pfx). It is still present in git history (commit edca21e) and must be treated as compromised: rotate the certificate in Azure/Cloudflare, and consider history rewrite + force-push + GitHub purge of cached forks/PRs. No infra or workflow references the file, so removal breaks nothing. **RESOLVED below in section 12.**

## 12. Certificate rotation + history purge (2026-09-15)

Findings and actions on the committed private key (storegrill-wildcard.pfx, blob 767319e1, added edca21e, removed 9871228):

- **Already rotated in Azure**: every regional pod is on a second-generation cert. UK `storegrill-uk-cert-v2` (CN storegrill.net), US/EU `storegrill-wildcard-cert-v2` (thumbprint E694553B78A6C3F7EA928C6BF38979A7764285FC), AE/NG apex `-v2` certs. The committed pfx is the retired v1 wildcard origin cert; no Azure mutation was required.
- **No Key Vault exposure**: the pfx was never stored as a Key Vault secret (vaults hold acs/postgres/stripe/paypal/jwt/vapid only). The file itself is password-protected with an unknown password.
- **History purge executed**: all branches rewritten with git filter-repo (--invert-paths --path storegrill-wildcard.pfx). Final HEAD tree identical to pre-rewrite tree (ce499345…); no pfx blob remains reachable from any branch. master + enterprise-refactor-session + feat/enterprise-email + fix/acs-mailer-hmac force-pushed. Local reflogs expired, gc --prune=now run, temp pfx copy deleted.
- **Untracked hygiene files in the same rewrite**: import-ng-secrets.cmd/.js/.ps1 (versioned Key Vault URIs) and all infra/terraform/live/**/terraform.tfvars (env + dev_client_ip) are no longer tracked; they remain on disk for local use and are ignored.
- **RESIDUAL**: GitHub still serves refs/pull/1..7 head refs pointing at pre-rewrite SHAs (contain the pfx) and maintains internal caches. Deleting those is not possible via git push; the repo owner must open a GitHub Support request (Purge key/cert from repository history) to purge cached views, pull-request refs and commit indexes. No key-revocation listing or account revival is needed on the Azure side — the rotation above already retired the key.
- **Impact**: public repo, zero forks, no open PRs (verified). Force-push re-ran CI/deploys with identical content (harmless). Commits after edca21e have new SHAs; any other clone of this repo will need to re-fetch (git fetch && git reset --hard origin/<branch>) or re-clone.
