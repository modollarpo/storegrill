                  ...
                  <tbody className="divide-y divide-border">
                    {cart.items.map(line => (
                      <tr key={`${line.productId}-${line.variantId ?? ''}`}>
                        ...
                        <td className="px-6 py-4">
                          <div className="inline-flex items-center justify-center rounded-xs border border-border bg-surface-sunken overflow-hidden mx-auto">
                            <button
                              type="button"
                        ...
                        <td className="px-6 py-4 text-center">
                          <button
                            ...
                            className="text-xs font-bold text-text-tertiary hover:text-red-600 hover:underline underline-offset-2 transition-colors"
                          >
                            Remove
                          </button>
                        </td>
          ...
          <aside aria-label="Order summary" data-testid="order-summary" className="bg-surface-raised border border-border rounded-lg overflow-hidden shadow-sm lg:sticky lg:top-28">
            ...
            <div className="p-6">
              ...
              <div className="mt-5 border-t border-border pt-4">
                ...
                >
                  Add a promo code
                  <span className={cn('text-text-tertiary group-hover:text-ember transition-colors text-lg font-bold', couponState !== 'idle' && 'text-ember')}>
                    {couponState !== 'idle' ? '−' : '+'}
                  </span>
                </button>
                {couponState !== 'idle' && (
                  <div className="mt-3 space-y-2">
                    {cart.appliedCoupon ? (
                      <div className="flex items-center justify-between gap-2 rounded-md border border-green-200 bg-green-50 px-3 py-2.5">
                        ...
                        <button type="button" onClick={removeCoupon} className="text-xs font-bold text-text-secondary hover:text-red-600 underline underline-offset-2 shrink-0">
                          Remove
                        </button>
                      </div>
                    ) : (
                      <>
                        <div className="flex gap-2">
                          ...
                          <button
                            ...
                            className="h-10 px-5 shrink-0 rounded-md border border-border text-sm font-bold text-text-primary hover:border-ember hover:text-ember transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                          >
  ...
  return (
    <label
      ...
      className={cn(
        ...
        checked ? 'border-ember bg-ember/5 shadow-sm' : 'border-border hover:border-ember'
      )}
    >
      <input
        id={id}
