...
export default function GlobalError({ reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <html lang="en">
      <body className="min-h-screen bg-surface-50 text-surface-900 font-sans antialiased">
        <main className="container-site min-h-screen flex flex-col items-center justify-center py-20 text-center">
          ...
          <button 
            type="button" 
            onClick={() => reset()} 
            className="inline-flex items-center justify-center h-12 px-10 rounded-full bg-surface-900 text-white font-bold tracking-wide hover:bg-surface-800 active:scale-95 transition-all shadow-md"
          >
            Refresh Application
          </button>
        </main>
